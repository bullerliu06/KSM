...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        [_bgView showRadius:10];
        [self.contentView addSubview:_bgView];
        [_bgView showShadow:COLORA(0, 0, 0, 0.1)];
        
        _topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _bgView.w, 40)];
        [_topView showRadius:10];
...
- (void)loadData:(NSDictionary *)dic {
    ...
    _status.textColor = [PPHelper colorFromHex:dic[p_orderStatusColor]];
    _topView.backgroundColor = [PPHelper colorFromHex:dic[p_orderStatusColor]];
    _topView.alpha = 0.2;
    ...
    amount.textColor = [PPHelper colorFromHex:dic[p_orderStatusColor]];
    ...
    if (repayTime.length > 0) {
        ...
        date.textColor = [PPHelper colorFromHex:dic[p_orderStatusColor]];
    ...
    if (!isBlankStr(btnString)) {
        ...
        if (bgColor.length > 0) {
            _selectBtn.backgroundColor = [PPHelper colorFromHex:bgColor];
            amount.textColor = [PPHelper colorFromHex:bgColor];
            date.textColor = [PPHelper colorFromHex:bgColor];
        }else {
