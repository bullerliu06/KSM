...
- (void)viewDidLoad {
    ...
    self.headerImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 228)];
    self.headerImageView.image = ImageWithName(@"user_header");
    [self.content addSubview:self.headerImageView];
...
- (void)loadHeader {
    [self.headerImageView removeAllViews];
    ...
    [self.headerImageView addSubview:setBtn];
    ...
    [self.headerImageView addSubview:name];
    ...
    [self.headerImageView addSubview:phoneNum];
    
    
    UIImageView *borrow = [[UIImageView alloc] initWithFrame:CGRectMake(33*WS, self.headerImageView.h - 83, 149*WS, 91)];
    ...
    [self.headerImageView addSubview:borrow];
    borrow.userInteractionEnabled = YES;

    UIImageView *order = [[UIImageView alloc] initWithFrame:CGRectMake(12*WS + borrow.right, self.headerImageView.h - 83, 149*WS, 91)];
    order.image = ImageWithName(@"user_order");
    order.userInteractionEnabled = YES;
    [self.headerImageView addSubview:order];
...
- (void)refreshUI {
    ...
    if (_refunPaydDic) {
        ...
        [payView showShadow:rgba(1, 38, 85, 0.10)];
        [self.listView addSubview:payView];
        [payView showRadius:12];
    ...
    UIView *itemView = [[UIView alloc] initWithFrame:CGRectMake(0, offsetY + 12, self.listView.w, self.itemDataList.count * 42 + 14)];
    ...
    for (int i = 0; i < self.itemDataList.count; i++) {
        NSDictionary *dic = self.itemDataList[i];
...
- (void)orderClick {
    ...
    [Page push:@"OrderPage" param:@{@"selectTabIndex":@(1), @"orderType":@"4"}];
}
...
- (void)itemAction:(UIButton *)sender {
    ...
    NSDictionary *dic = self.itemDataList[sender.tag];
...
- (void)borrowClick {
    ...
    [Page push:@"OrderPage" param:@{@"selectTabIndex":@(0), @"orderType":@"6"}];
}
...
- (void)catchUserData {
    ...
    [Http get:R_userCenter params:nil success:^(Response *response) {
        if (response.success) {
            weakSelf.itemDataList = [NSMutableArray arrayWithArray:response.dataDic[p_extendLists]];
