...
- (void)viewDidLoad {
    ...
    [backButton showRadius:24];
    backButton.backgroundColor = MainColor;
    [self.content addSubview:backButton];
    [backButton addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];

    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(counting:) name:CountDownLogin object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(countingFinishCompleted) name:CountDownLoginFinish object:nil];
...
- (void)sureAction {
    NSString *code =  self.codeView.text;
    if (code.length != 6) {
        [PPToast show:@"Please enter valid verification code"];
        return;
    }
    NSDictionary *dic = @{
...
- (NSDictionary *)track {
    NSDictionary *pointDic = @{@"rsgothicCiopjko": self.startTime,
                               ...
                               @"rsnonallergenicCiopjko":[PPPhoneInfo idfv],
