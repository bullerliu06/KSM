...
@interface InformationPage () <UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) UIView *header;
@property (nonatomic, strong) UIView *emailTypeView;

@property (nonatomic, copy) NSMutableArray *dataItemList;
...
@implementation InformationPage
...
- (void)requestInfoData {
    ...
    [Http post:R_personal_info params:dic success:^(Response *response) {
        if (response.success) {
            weakSelf.dataItemList = [NSMutableArray arrayWithArray:response.dataDic[p_items]];
...
- (void)loadUI {
    ...
    for (NSDictionary *info in self.dataItemList) {
    ...
    UIButton *next = [HView normalBtn:CGRectMake(34, self.view.h - SafeBottomHeight - 60, ScreenWidth - 68, 48) title:@"Next" font:24];
    ...
    if (self.emailY > 0) {
        _emailTypeView = [[UIView alloc] initWithFrame:CGRectMake(LeftMargin, self.emailY, SafeWidth, 4*30)];
        _emailTypeView.hidden = YES;
        _emailTypeView.backgroundColor = UIColor.whiteColor;
        [_emailTypeView showRadius:8];
        [_emailTypeView showShadow:LineGrayColor];
        [self.content addSubview:_emailTypeView];
    }
}

- (void)nextAction {
    for (NSDictionary *info in _dataItemList) {
        NSArray *itemArr = info[p_list];
        for (int i = 0; i < itemArr.count; i++) {
            NSDictionary *dic = itemArr[i];
            NSString *codeString = dic[p_code];
            NSString *titleString = dic[p_title];
            NSString *valueStr = self.saveDic[codeString];
...
- (void)saveData {
    ...
    kWeakself;
    [Http post:R_save_personal_info params:self.saveDic success:^(Response *response) {
        if (response.success) {
...
- (TimeView*)timeView {
    if(!_timeView) {
        ...
        _timeView.finishBlock = ^{
            ...
            weakSelf.emailTypeView.y = weakSelf.emailY - 44;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSDictionary *dic = _dataItemList[indexPath.section][p_list][indexPath.row];
    NSString *cate = dic[p_cate];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self.view endEditing:YES];
    NSDictionary *cellDic = _dataItemList[indexPath.section][p_list][indexPath.row];
    ...
    if ([cate isEqualToString:p_enum]) {
        ...
        InfoAlert *alert = [[InfoAlert alloc] initWithData:noteList selected:[_saveDic[code] stringValue] title:title];
        alert.selectBlock = ^(NSDictionary *dic) {
            ...
            weakSelf.saveDic[code] = notNull([dic[p_type] stringValue]);
            ...
            if ((currentSection + 1) < [tableView numberOfSections] && ([tableView numberOfRowsInSection:currentSection] > 0)) {
                NSDictionary *nextDic = weakSelf.dataItemList[nextPath.section][p_list][nextPath.row];
                NSString *code = nextDic[p_code];
                NSString *value = [weakSelf.saveDic[code] stringValue];
                if (value.length == 0) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    NSDictionary *dic = _dataItemList[section];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return _dataItemList.count;
}


- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (section < _dataItemList.count - 1) {
        return 38;
    }else {
        return 82;
    }
