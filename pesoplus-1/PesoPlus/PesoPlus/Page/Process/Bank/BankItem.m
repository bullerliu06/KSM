...
@interface BankItem () <UITextFieldDelegate>
@property (nonatomic, strong) UITextField *bankTextField;
@property (nonatomic, strong) UITextField *accountTextField;
@property (nonatomic, copy) NSArray *bankItemList;
...
- (void)loadUI {
    ...
    _bankTextField = [[UITextField alloc] initWithFrame:CGRectMake(self.w - 140, titleLabel.bottom + 14, 100, 42)];
    _bankTextField.borderStyle = UITextBorderStyleNone;
    _bankTextField.textAlignment = NSTextAlignmentRight;
    _bankTextField.clearButtonMode = UITextFieldViewModeWhileEditing;
    _bankTextField.enabled = NO;
    _bankTextField.font = Font(12);
    
    _bankTextField.textColor = rgba(51, 51, 51, 1);
    _bankTextField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;

    _bankTextField.placeholder = @"Please select";
    [self addSubview:_bankTextField];
    
    UIImageView *right = [[UIImageView alloc] initWithFrame:CGRectMake(_bankTextField.right + 10, _bankTextField.y + 13, 16, 16)];
    ...
    [_accountTextField showRadius:10];
...
- (void)bankSelect:(UIButton *)sender {
    kWeakself;
    [self endEditing:YES];
    _alert = [[InfoAlert alloc] initWithData:self.bankItemList selected:[_saveDic[p_channel] stringValue] title:@"Select Bank"];
    _alert.selectBlock = ^(NSDictionary *dic) {
        weakSelf.bankTextField.text = notNull(dic[p_name]);
...
- (void)refreshData {
    ...
    for (int i = 0; i < self.bankItemList.count; i++) {
        NSDictionary *dic = self.bankItemList[i];
        NSInteger channel = [dic[p_id] integerValue];
        if (channel == selectChannel) {
            _bankTextField.text = notNull(dic[p_name]);
