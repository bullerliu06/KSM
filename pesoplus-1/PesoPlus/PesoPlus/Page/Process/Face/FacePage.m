...
- (void)loadData {
    ...
    [Http post:R_facial params:dic success:^(Response *response) {
        if (response.success) {
            weakSelf.saveDic = [NSMutableDictionary dictionaryWithDictionary:response.dataDic[p_card_two]];
            [weakSelf loadUI];
        }
...
- (void)saveFaceInfo:(NSString *)value {
...
- (void)viewDidLoad {
    ...
    _nextItem = [HView normalBtn:CGRectMake(34, self.view.h - SafeBottomHeight - 60, ScreenWidth - 68, 48) title:@"Start" font:24];
    [_nextItem setTitle:@"Try Again" forState:UIControlStateSelected];
    [self.view addSubview:_nextItem];
    [_nextItem showBottomShadow:COLORA(0, 0, 0, 0.2)];
    [_nextItem addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(nextAction)]];
...
- (void)loadUI {
    ...
    if (_isError) {
        IconTitleView *wrongBtnItem = [[IconTitleView alloc] initWithFrame:CGRectMake(25, offsetY, ScreenWidth - 50, 42) title:@"Authentication failed,please try again" color:UIColor.whiteColor font:14 icon:@"icon_tips"];
        wrongBtnItem.backgroundColor = rgba(255, 122, 122, 1);
        [wrongBtnItem showRadius:21];
    ...
    _nextItem.selected = _isError;
}

- (void)nextAction {
    NSString *faceId = self.saveDic[p_value];
    if (faceId.length > 0) {
        [self saveFaceInfo:faceId];
        return;
    }
    [self configAASdk];
