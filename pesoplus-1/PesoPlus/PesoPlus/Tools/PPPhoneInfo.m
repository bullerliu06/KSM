...
{
    return notNull([self deviceName]);
}
...
+ (NSString *)idfv {
    NSString *idfv = [IDFVTools getIDFV];
    return notNull(idfv);
}
...
+ (NSString*)firstInstallation
{
    ...
    NSString *fistTimeStr = [PPCache getStr:KEY_FistTime];
    
    if (isBlankStr(fistTimeStr))
    {
        NSString *fistTimeStr = [self nowTimeString];
        [PPCache setStr:fistTimeStr forKey:KEY_FistTime];
        return fistTimeStr;
    }
    else{
        return fistTimeStr;
