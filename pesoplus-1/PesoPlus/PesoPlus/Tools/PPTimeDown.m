        ...
        if (timeInter <= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ([tempTimer isEqual:self.loginTimer]) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:CountDownLoginFinish object:@(timeInter)];
                }
            });
        }else {
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([tempTimer isEqual:self.loginTimer]) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:CountDownLogin object:@(timeInter)];
                }
            });
        }
    });
