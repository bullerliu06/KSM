...
@interface PPBasePage () <UIGestureRecognizerDelegate>
@end

@implementation PPBasePage
...
- (PPContentView *)content {
    if (!_content) {
        CGFloat height = [self contentHeight];
        _content = [[PPContentView alloc] initWithFrame:CGRectMake(0, self.naviBarHidden ? 0 : NavBarHeight, ScreenWidth, height)];
...
- (PPNavBar *)navBar {
    if (!_navBar) {
        _navBar = [[PPNavBar alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, NavBarHeight)];
        kWeakself;
