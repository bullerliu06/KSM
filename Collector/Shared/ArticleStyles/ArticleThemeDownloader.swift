...
struct ArticleThemeDownloader {
	...
	public enum ArticleThemeDownloaderError: LocalizedError {
		...
		public var errorDescription: String? {
			switch self {
			case .noThemeFile:
				return "There is no Collector theme available."
			}
		}
	}
	...
	/// Removes downloaded themes, where themes == folders, from `Application Support/Collector/Downloads`.
	static func cleanUp() {
...
private extension ArticleThemeDownloader {
	...
	/// Creates `Application Support/Collector/Downloads` if needed.
	...
	static func unzipFile(at location: URL) throws -> URL {
		do {
			...
			try Zip.unzipFile(location, destination: unzipDirectory, overwrite: true, password: nil, progress: nil, fileOutputHandler: nil) // Unzips to folder in Application Support/Collector/Downloads
	...
	/// The download directory used by the theme downloader: `Application Support/Collector/Downloads`
	/// - Returns: `URL`
	static func downloadDirectory() -> URL {
		FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("Collector/Downloads", isDirectory: true)
	}
}
