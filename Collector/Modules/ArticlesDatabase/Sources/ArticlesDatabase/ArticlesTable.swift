...
//  
//
//  Created by Solid Omit on 3/10/24.
//
...
final class ArticlesTable {
	...
	func articles(feedIDs: Set<String>, database: FMDatabase) -> Set<Article> {
		...
		// select * from articles natural join statuses where feedID in ('http://sotech.com/xml/rss.xml') and read=0
	...
	func starredArticles(feedIDs: Set<String>, limit: Int?, database: FMDatabase) -> Set<Article> {
		...
		// select * from articles natural join statuses where feedID in ('http://sotech.com/xml/rss.xml') and starred=1;
...
private extension ArticlesTable {
	...
	func fetchArticlesSince(feedIDs: Set<String>, cutoffDate: Date, limit: Int?, database: FMDatabase) -> Set<Article> {
		...
		// select * from articles natural join statuses where feedID in ('http://sotech.com/xml/rss.xml') and (datePublished > ? || (datePublished is null and dateArrived > ?)
		//
