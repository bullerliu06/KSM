...
@MainActor public final class Account: DisplayNameProvider, UnreadCountProvider, Container, Hashable {
	...
	private lazy var opmlFile = OPMLFile(filename: (dataFolder as NSString).appendingPathComponent("localdata.opml"), account: self)
	...
	public func debugRunSearch() {
		...
		Task {
			let t1 = Date()
			let articles = try! await articlesMatching(searchString: "Brent Collector")
			let t2 = Date()
